...
DIR_TEST_FILES = os.path.join(DIR_ROOT, 'public_tests')
...
DIR_BUILD = os.path.join(DIR_PROJECT, 'src')
...
def run_tests(methods, exe_path, test_dir, output_dir, timeout=180.0):
    ...
    for method in methods:
        ...
        for test_file in sorted(Path(test_dir).rglob('*')):
            test_path_parent = str(test_file.parent.resolve())
...
def main():
    ...
    exe_path = os.path.join(DIR_PROJECT, 'compress.py')
